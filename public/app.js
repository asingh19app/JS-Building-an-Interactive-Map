...
//Now it's time to add the local metro stops to the map. Leaflet offers a method called layerGroup for grouping multiple markers. We could add all the stops individually, but doing so will create a new layer for every stop (multiple if using shadows for pins). While this may not seem like a problem, if you want to add and remove sets of pins, it becomes difficult targeting specific pins across multiple layers.
